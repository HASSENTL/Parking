...
void on_but1modifier_clicked(GtkButton *button, gpointer user_data) {
    ...
    // Récupération de l'ID sélectionné dans le GtkComboBoxEntry
    GtkTreeIter iter;
    GtkTreeModel *model = gtk_combo_box_get_model(GTK_COMBO_BOX(idmodif));
    gchar *idmodiftext = NULL;

    if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(idmodif), &iter)) {
        gtk_tree_model_get(model, &iter, 0, &idmodiftext, -1); // Récupérer l'ID sélectionné
        strcpy(idparkmodif, idmodiftext);
        g_free(idmodiftext); // Libérer la mémoire allouée par gtk_tree_model_get
    } else {
        g_warning("Aucun ID sélectionné !");
        gtk_label_set_text(GTK_LABEL(modiflabel), "Veuillez sélectionner un parking !");
        return;
    }
    ...
    strcpy(adrmodif, gtk_entry_get_text(GTK_ENTRY(adressemodif))); // Correction ici

    // Création du nouveau parking
    parking nouv;
    ...
    if (tr == 0) { 
        g_warning("Parking non modifié");
        gtk_label_set_text(GTK_LABEL(modiflabel), "Échec de la modification !");
        return;
    }

    if (tr == 1) {
        g_warning("Parking modifié avec succès");
        gtk_label_set_text(GTK_LABEL(modiflabel), "Parking modifié avec succès !");
        return;
    }
remplir_combobox_ids(idmodif);
}
...
GtkWidget *idmodif = lookup_widget(modifierparking, "idmodif");
remplir_combobox_ids(idmodif);
gtk_widget_hide(gererparking);
